...
public class FeatureInfusionProvider extends ThEThaumcraftResearchFeature {
    ...
    protected void registerCrafting(final CommonDependantItems cdi) {
        ...
        ItemStack[] infusionProviderRecipeItems = {
            cdi.EssentiaMirror,
            cdi.SalisMundus,
            CoalescenceCore,
            cdi.AirShard,
    ...
    protected void registerResearch() {
        ...
        String pseudoKeyMirrorOrJar =
                (Config.allowMirrors ? PseudoResearchTypes.MIRROR.getKey() : PseudoResearchTypes.JAR.getKey());

        // Create the infusion provider research
        ResearchTypes.INFUSION_PROVIDER.createResearchItem(
                infusionProviderList, ResearchRegistry.COMPLEXITY_LARGE, infusionProviderIcon, infusionProviderPages);
        ResearchTypes.INFUSION_PROVIDER.researchItem.setParents(
                this.getFirstValidParentKey(false), pseudoKeyMirrorOrJar, PseudoResearchTypes.INFUSION.getKey());
